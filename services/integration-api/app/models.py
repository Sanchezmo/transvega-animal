...
class Breed(Base):
    ...
    __table_args__ = (UniqueConstraint("name", name="uq_breed_name"),)
...
class DogStatusHistory(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('draft', 'available', 'reserved', 'sold', 'inactive')",
            name="ck_dog_status_history_status",
        ),
    )
...
class Publication(Base):
    ...
    )  # draft, pending_approval, approved, published, expired, removed, failed
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('draft', 'pending_approval', 'approved', 'published', 'expired', 'removed', 'failed')",
            name="ck_publication_status",
        ),
        CheckConstraint(
            "platform IN ('web', 'milanuncios', 'facebook', 'instagram', 'tiktok')",
            name="ck_publication_platform",
        ),
    )
