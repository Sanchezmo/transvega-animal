...
class ProductsAgent:
    ...
    async def _create_expediente_draft(self, data: Dict) -> Dict:
        ...
        # Validaciones
        required_fields = ["name", "breed", "sex", "birth_date", "color", 
                          "weight_kg", "microchip", "breeder_id"]
    ...
    async def _validate_expediente(self, data: Dict) -> Dict:
        ...
        validations["documents_complete"] = all(
            expediente.get(doc) for doc in required_docs
        )
    ...
    async def _detect_missing_fields(self, data: Dict) -> Dict:
        ...
        required = [
            "name", "breed", "sex", "birth_date", "color", 
            "weight_kg", "microchip", "breeder_id",
            "breeder_registration", "zoological_nucleus",
            "country_origin", "sire_name", "dam_name",
            "pedigree", "vet_status", "vaccines", "deworming",
            "passport", "certificates", "photos", "purchase_price",
            "sale_price", "associated_costs",
        ]
        
        optional = [
            "place_origin", "certificates", "videos",
            "associated_costs", "reservation_id", "order_id",
            "invoice_id", "transport_id",
        ]
    ...
    def _generate_description(self, exp: Dict) -> str:
        ...
        parts = [
            ...
            "",
            "#perros #cachorros #" + exp.get("breed", "").lower().replace(" ", "") + " #pedigree #LOE #FCI #garantia #envio #adopcionresponsable #criaderoselectivo #transvegaanimal"
        ]
    ...
    def _generate_hashtags(self, exp: Dict) -> List[str]:
        ...
        base = ["#perros", "#cachorros", "#adopcionresponsable", "#compraresponsable", 
                "#bienestaranimal", "#transvegaanimal", "#criaderoselectivo"]
    ...
    async def _propose_categories(self, data: Dict) -> Dict:
        ...
        categories = {
            ...
            "sex": exp.get("sex", "").lower(),
            "size": self._estimate_size(exp.get("breed", ""), exp.get("weight_kg", 0)),
            "age_group": "cachorro",
    ...
    def _estimate_size(self, breed: str, weight: float) -> str:
        ...
        elif weight > 15: return "medium"
        elif weight > 8: return "small"
        return "toy"
...
def get_products_agent(config: Optional[Dict] = None):
...
async def start_products_agent(config: Optional[Dict] = None):
