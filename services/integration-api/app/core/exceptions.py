...
class AuthorizationException(TransvegaException):
    ...
    def __init__(self, message: str = "Permisos insuficientes", details: Optional[Dict] = None):
        super().__init__(
            message=message,
...
class IdempotencyException(TransvegaException):
    ...
    def __init__(self, key: str, existing_id: str, details: Optional[Dict] = None):
        super().__init__(
            message=f"Operación duplicada detectada para clave: {key}",
            error_code="IDEMPOTENCY_CONFLICT",
            status_code=409,
            details={**{"idempotency_key": key, "existing_resource_id": existing_id}, **(details or {})},
        )
...
class DolibarrException(TransvegaException):
    ...
    def __init__(self, message: str, endpoint: str = "", status_code: int = 502, details: Optional[Dict] = None):
        super().__init__(
...
class ApprovalRequiredException(TransvegaException):
    ...
    def __init__(self, action: str, approval_id: str, details: Optional[Dict] = None):
        super().__init__(
