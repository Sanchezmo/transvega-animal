...
class MarketingAgent:
    ...
    async def _propose_campaign(self, data: Dict) -> Dict:
        ...
        return {
            "success": True,
            "campaign": campaign,
            "message": "Campaña propuesta, pendiente de aprobación" if campaign["requires_approval"] else "Campaña aprobada automáticamente",
        }
    ...
    async def _propose_improvements(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "priority_order": sorted(improvements, key=lambda x: {"high": 3, "medium": 2, "low": 1}[x["impact"]], reverse=True),
        }
...
#perros #{exp.get('breed', '').lower().replace(' ', '')} #cachorros #adopcionresponsable #criaderoselectivo #pedigree #transvegaanimal""",
            "hashtags": ["perros", "cachorros", "adopcionresponsable", "criaderoselectivo", "pedigree", "transvegaanimal"],
        }
    ...
    async def _ig_reel_template(self, data: Dict) -> Dict:
        ...
        return {
            "title": f"{exp.get('name', 'Cachorro')} - {exp.get('breed', 'Raza')} 🐾",
            "caption": f"{exp.get('name', 'Cachorro')} de {exp.get('breed', 'raza')} con {exp.get('age_months', 2)} meses 🐶\n\nPadres: {exp.get('sire_name', 'Campeón')} + {exp.get('dam_name', 'Campeona')}\n\n✅ LOE/FCI • Vacunas • Chip • Garantías\n💰 {exp.get('sale_price', 0)}€\n📩 WhatsApp +34 XXX XX XX XX\n\n#perros #cachorros #{exp.get('breed', '').lower().replace(' ', '')} #adopcionresponsable #transvegaanimal",
    ...
    async def _fb_post_template(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "body": f"""{exp.get('name', 'Nuestro cachorro')} de {exp.get('breed', 'raza')} busca su hogar definitivo 🏠❤️
...
🐕 Padre: {exp.get('sire_name', 'Campeón')}
🐕 Madre: {exp.get('dam_name', 'Campeona')}

🚚 Entrega: Recogida en criadero O envío nacional/internacional (IATA)
💰 Precio: {exp.get('sale_price', 0)}€ (reserva 30%)

📩 MÁS INFO: WhatsApp +34 XXX XX XX XX
🌐 transvega-animal.es

#perros #cachorros #adopcionresponsable #criaderoselectivo #{exp.get('breed', '').lower().replace(' ', '')} #pedigree #LOE #FCI""",
        }
...
*¿Buscas un cachorro de raza? En Transvega Animal te acompañamos en todo el proceso.*""",
            ...
            "seo_description": f"Descubre cómo elegir un cachorro responsablemente. Consejos de expertos en cría selectiva.",
        }
        ...
        return {
            ...
            "sections": [
                ...
                },
                {
                    ...
                    "content": "\"Luna llegó perfecta, sana y super socializada. El seguimiento post-entrega es increíble.\"",
                },
                {
                    "type": "tip",
                    "title": "💡 Tip de la semana",
                    "content": "La socialización entre 3-16 semanas es CRÍTICA. Expón a tu cachorro a sonidos, personas, superficies y otros perros de forma positiva.",
                },
            ],
            "footer": {
    ...
    def _generate_hashtags(self, topic: str, platform: str) -> List[str]:
        base = ["transvegaanimal", "perros", "cachorros", "adopcionresponsable", "criaderoselectivo", "pedigree", "LOE", "FCI"]
