...
def enviar_notificacion_aprobacion(self, aprobacion_id: str, accion: str, destinatarios: list[str], datos: dict):
    """
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def notificar_resultado_aprobacion(
    self, aprobacion_id: str, aprobada: bool, solicitante_id: str, comentario: str = None
):
    """
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def notificar_cambio_estado_expediente(
    self, expediente_id: int, estado_anterior: str, estado_nuevo: str, destinatarios: list[str]
):
    """
    Notificar cambio de estado de expediente.
    """
    logger.info(
        "notificando_cambio_estado",
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def notificar_factura_vencida(self, factura_id: int, dias_vencida: int, cliente_email: str, importe: float):
    """
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def notificar_entrega_programada(
    self, expediente_id: int, fecha_entrega: str, transportista: str, tracking_url: str, destinatarios: list[str]
):
    """
...
@shared_task(bind=True, max_retries=3, default_retry_delay=60)
def notificar_alerta_sistema(self, nivel: str, mensaje: str, servicio: str, destinatarios: list[str]):
    """
