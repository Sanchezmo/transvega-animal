...
# Ads metrics
ADS_FOUND = Counter(
    "milanuncios_ads_found_total",
    "Total ads found on page"
)

ADS_RENEWED = Counter(
    "milanuncios_ads_renewed_total",
    "Total ads successfully renewed"
)
...
ADS_SKIPPED = Counter(
    "milanuncios_ads_skipped_total",
    "Total ads skipped (already renewed, not eligible, etc.)"
)

# Duration metrics
RENEWAL_DURATION = Histogram(
    "milanuncios_renewal_duration_seconds",
    "Time to renew a single ad",
    buckets=[1, 2, 5, 10, 20, 30, 60, 120, 300]
)

RUN_DURATION = Histogram(
    "milanuncios_run_duration_seconds",
    "Total run duration",
    buckets=[10, 30, 60, 120, 300, 600, 1200, 1800]
)

LOGIN_DURATION = Histogram(
    "milanuncios_login_duration_seconds",
    "Time to complete login",
    buckets=[1, 2, 5, 10, 15, 20, 30, 60]
)

# CAPTCHA metrics
CAPTCHA_HITS = Counter(
    "milanuncios_captcha_hits_total",
    "CAPTCHA challenges detected"
)

# State gauge
ACTIVE_ADS = Gauge(
    "milanuncios_active_ads",
    "Number of active ads in database"
)
...
CURRENT_ADS_RENEWING = Gauge(
    "milanuncios_current_ads_renewing",
    "Number of ads currently being renewed"
)

# Browser metrics
BROWSER_START_DURATION = Histogram(
    "milanuncios_browser_start_duration_seconds",
    "Time to start browser context"
)

BROWSER_ERRORS = Counter(
    "milanuncios_browser_errors_total",
