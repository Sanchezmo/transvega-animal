...
class PurchasesAgent:
    ...
    async def _parse_invoice_document(self, data: dict) -> dict:
        ...
        _ = data.get("mime_type", "application/pdf")
    ...
    async def _detect_duplicates(self, data: dict) -> dict:
        ...
        _ = invoice_data.get("vendor_vat")
        _ = invoice_data.get("invoice_number")
        _ = invoice_data.get("date")
    ...
    async def _create_draft_purchase_invoice(self, data: dict) -> dict:
        ...
        #     return {
        #         "success": False,
        #         "error": "Proveedor nuevo - requiere alta manual",
        #         "requires_vendor_creation": True,
        #     }
    ...
    async def _detect_incoherences(self, data: dict) -> dict:
        ...
        calculated_ht = sum(
            Decimal(str(line.get("qty", 0)))
            * Decimal(str(line.get("unit_price", 0)))
            * (Decimal("1") - Decimal(str(line.get("discount", 0))) / Decimal("100"))
            for line in lines
        )
        ...
        calculated_vat = sum(
            ...
            * (Decimal(str(line.get("vat_rate", 21))) / Decimal("100"))
            for line in lines
        )
    ...
    async def _request_human_review(self, data: dict) -> dict:
        ...
        _ = data.get("reason", "Revisión requerida por agente")
        _ = data.get("priority", "normal")
