...
class GoogleWorkspaceClient:
    ...
    async def gmail_list_messages(self, query: str = "", max_results: int = 100, label_ids: List[str] = None) -> List[Dict]:
    ...
    async def drive_upload_file(self, file_content: bytes, filename: str, folder_id: str = "root", mime_type: str = None) -> Dict:
    ...
    async def calendar_list_events(self, calendar_id: str = "primary", time_min: str = None, time_max: str = None, max_results: int = 100) -> List[Dict]:
    ...
    async def calendar_update_event(self, calendar_id: str = "primary", event_id: str = None, event_data: Dict = None) -> Dict:
        """Actualizar evento."""
        token = await self._get_access_token()
...
class GoogleWorkspaceAdapter:
    ...
    async def process_invoice_emails(self, label: str = "FACTURAS_PROVEEDORES") -> List[Dict]:
        ...
        invoices = []
        for msg in messages:
            message = await self.client.gmail_get_message(msg["id"])
    ...
    async def backup_dolibarr_documents(self, folder_name: str = "Dolibarr Backups") -> Dict:
        ...
        folders = await self.client.drive_list_files(query=f"name='{folder_name}' and mimeType='application/vnd.google-apps.folder'")
    ...
    async def sync_calendar_appointments(self, appointments: List[Dict]) -> Dict:
        ...
        for appt in appointments:
            event = {
                ...
                "end": {"dateTime": (datetime.fromisoformat(appt["scheduled_at"]) + timedelta(minutes=appt.get("duration_minutes", 30))).isoformat(), "timeZone": "Europe/Madrid"},
