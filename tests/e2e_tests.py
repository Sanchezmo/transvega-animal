...
async def test_create_and_get_expediente():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        resp = await ac.post("/api/v1/expedientes/expedientes", json=payload, headers=headers)
        assert resp.status_code == 201
        data = resp.json()
        expediente_id = data["id"]

        resp2 = await ac.get(f"/api/v1/expedientes/expedientes/{expediente_id}", headers=headers)
...
async def test_invoice_flow():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        resp = await ac.post("/api/v1/terceros/terceros", json=tp, headers=headers)
        assert resp.status_code == 201
        ...
        resp2 = await ac.post("/api/v1/facturacion/facturas", json=inv, headers=headers2)
