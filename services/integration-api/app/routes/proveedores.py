...
from app.dependencies.auth import get_current_agent, require_financial, require_write
...
async def list_proveedores(
    ...
    sqlfilters: str | None = Query(default=None, description="Filtros SQL adicionales"),
...
async def get_proveedor(
    ...
    try:
        ...
        raise NotFoundException(f"Proveedor {proveedor_id} no encontrado: {e!s}")
...
async def update_proveedor(
    ...
    proveedor = await dolibarr.update_supplier(
        proveedor_id, proveedor_data.model_dump(exclude_none=True, exclude_unset=True)
    )
    return proveedor
...
async def update_supplier_invoice(
    ...
    invoice = await dolibarr.update_supplier_invoice(
        invoice_id, invoice_data.model_dump(exclude_none=True, exclude_unset=True)
    )
    return invoice
...
async def update_supplier_proposal(
    ...
    proposal = await dolibarr.update_supplier_proposal(
        proposal_id, proposal_data.model_dump(exclude_none=True, exclude_unset=True)
    )
    return proposal
