...
class SalesAgent:
    ...
    async def _register_inquiry(self, data: Dict) -> Dict:
        ...
        valid_sources = ["web", "whatsapp", "email", "phone", "milanuncios", "facebook", "instagram", "tiktok", "referral", "show"]
    ...
    async def _auto_qualify_from_expediente(self, lead: Dict, expediente_id: str) -> Dict:
        ...
        lead["intent"] = "show_breeding" if expediente.get("breed") in ["Golden Retriever", "Pastor Alemán"] else "companion"
        
        return lead
    
    def _calculate_temperature(self, score: int) -> str:
        if score >= 81: return "burning"
        elif score >= 61: return "hot"
        elif score >= 31: return "warm"
        return "cold"
    ...
    async def _qualify_lead(self, data: Dict) -> Dict:
        ...
        if "status" in qualification:
            valid_statuses = ["new", "contacted", "qualified", "proposal_sent", "negotiation", "won", "lost", "nurturing"]
        ...
        # Perfil hogar
        profile_fields = [
            "housing_type", "hours_alone", "has_children", "children_ages",
            "has_dogs", "current_dogs", "has_cats", "experience_level", "show_experience"
        ]
    ...
    def _recalculate_score(self, qualification: Dict, existing: Dict) -> int:
        ...
        if qualification.get("budget_max"):
            ...
            if budget >= 8000: score += 25
            elif budget >= 5000: score += 20
        ...
        # Bonificaciones
        if qualification.get("show_experience"): score += 5
        if qualification.get("has_dogs") and qualification.get("experience_level") in ["criador", "handler"]: score += 3
    ...
    async def _create_quote(self, data: Dict) -> Dict:
        ...
        total_ht = sum(Decimal(str(l["unit_price"])) for l in lines)
        total_tva = sum(Decimal(str(l["unit_price"])) * Decimal("0.21") for l in lines)
    ...
    async def _send_follow_up(self, data: Dict) -> Dict:
        ...
        message = custom_message or templates.get(template, templates["generic"])
        
        # Obtener datos del lead
        # lead = await db.get_lead(lead_id)
        
        # Formatear mensaje
        # message = message.format(name=lead.first_name, breed=lead.preferred_breeds[0] if lead.preferred_breeds else "cachorro")
    ...
    async def _answer_faq(self, data: Dict) -> Dict:
        ...
        faqs = {
            "precio": "Los precios varían por raza, línea de sangre y servicios incluidos. Van desde 1.200€ a 3.000€ + transporte.",
            "transporte": "Ofrecemos transporte nacional (24-48h) e internacional (3-7 días). Vehículos IATA, conductores certificados.",
            "garantia": "Garantía genética de por vida (displasia, ojos, corazón, ADN), víricas 14 días, temperamento 1 año.",
            "pago": "Reserva 30% al confirmar, resto a la entrega. Transferencia, tarjeta o financiación.",
            "documentacion": "Entregamos: LOE/FCI, cartilla vacunal, microchip, certificado vet, contrato, guía, kit cachorro.",
            "visita": "¡Claro! Agendamos visita al criadero (presencial o videollamada). Verás padres, instalaciones y cachorros.",
            "exportacion": "Exportamos a LatAm, EE.UU., Europa. Gestionamos CITES, certificado zoosanitario, trámites aduaneros.",
    ...
    async def _request_missing_data(self, data: Dict) -> Dict:
        ...
        message = "Para poder ayudarte mejor, necesito saber: " + ", ".join(
