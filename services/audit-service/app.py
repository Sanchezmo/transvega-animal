...
class AuditService:
    ...
    async def log_event(self, event: dict) -> dict:
        ...
        if event.get("request_body"):
            request_body_hash = hashlib.sha256(json.dumps(event["request_body"], sort_keys=True).encode()).hexdigest()

        new_state_hash = None
        if event.get("new_state"):
            new_state_hash = hashlib.sha256(json.dumps(event["new_state"], sort_keys=True).encode()).hexdigest()
        ...
        current_hash = hashlib.sha256(f"{previous_hash}{event_id}{datetime.now().isoformat()}".encode()).hexdigest()
...
def get_audit_service(config: dict | None = None) -> "AuditService":
...
async def start_audit_service(config: dict | None = None):
    global _audit_service
    _audit_service = AuditService(config or {})
