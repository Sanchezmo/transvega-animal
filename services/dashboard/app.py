...
async def expedientes_page(request: Request, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def expediente_detail(request: Request, expediente_id: int, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def aprobaciones_page(request: Request, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def aprobar_aprobacion(
    approval_id: str, decision: ApprovalDecision, user: dict | None = Depends(get_current_user)
):
...
async def comercial_page(request: Request, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def facturacion_page(request: Request, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def monitoreo_page(request: Request, user: dict | None = Depends(get_current_user)):
    if not user:
        return templates.TemplateResponse("login.html", {"request": request}, status_code=302)
...
async def dashboard_stats(user: dict | None = Depends(get_current_user)):
    if not user:
...
async def api_aprobaciones_pendientes(user: dict | None = Depends(get_current_user)):
    if not user or user["role"] not in ["admin", "supervisor"]:
